          ...
          if current_time - last_key_time >= key_timeout:
              if key == ord('q'):
                  ...
                  if name:
                      ...
                      time.sleep(1)  # Aguarda um momento para capturar um frame mais estável
                  ...
                  name = input("\n👤 Digite o nome da pessoa: ").strip()
                  if name:
                      directory = input("📁 Digite o caminho do diretório com as fotos: ").strip()
                      if os.path.exists(directory):
                          ...
                          for filename in os.listdir(directory):
                              if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                                  ...
                                  dst_path = os.path.join(person_dir, f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{filename}")
                                  try:
      ...
      if not training_in_progress:
          ...
          if current_time - last_key_time >= key_timeout:
              if key == ord('q'):
                  ...
                  print("\n🔄 Iniciando treinamento...")
                  cv2.destroyAllWindows()  # Fecha a janela da câmera durante o treinamento
                  training_in_progress = True
                  success = face_system.train_model()
                  training_in_progress = False
                  if success:
                      print("✅ Treinamento concluído!")
                      print("ℹ️ Pressione 's' para salvar o modelo")
                  cv2.namedWindow('Reconhecimento Facial')  # Recria a janela da câmera
                  ...
                  if name:
                      print("📸 Capturando face... Olhe para a câmera.")
                      # Aguarda um momento para capturar um frame mais estável
                      time.sleep(1)
                      ret, capture_frame = cap.read()
                      if ret:
                          gray = cv2.cvtColor(capture_frame, cv2.COLOR_BGR2GRAY)
